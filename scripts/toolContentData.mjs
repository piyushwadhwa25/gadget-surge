...
export const toolContentMap = {
  ...
  'base64-encoder': {
    ...
    faqs: [
      ...
      { question: 'How do I convert text to Base64 online?', answer: 'Paste or type your text into the input field on this page, then click Encode. The tool converts the text to a Base64 string instantly and shows the result in the output field. Use Copy to grab the encoded value for headers, data URIs, or API payloads — no account or download required.' },
      { question: 'Can I convert a string to Base64 without installing anything?', answer: 'Yes. This encoder is fully browser-based, so you do not need to install a CLI tool, IDE plugin, or desktop app. Open the page, paste your string, and encode it locally in JavaScript. Nothing is uploaded, and you can use it from any modern browser.' },
      { question: 'Is there a difference between \'encode to Base64\' and \'convert to Base64\'?', answer: 'No. Encode to Base64 and convert to Base64 describe the same operation: transforming raw text or bytes into a Base64 ASCII representation. People search both phrases, but the output is identical. This tool performs that conversion — paste text, encode, and copy the Base64 result.' },
      { question: 'How do I encode a string to Base64 for an API?', answer: 'Paste the exact string your API expects (for example username:password for Basic Auth, or a raw JSON fragment) into the input, click Encode, then copy the Base64 output into your Authorization header or request body. Because encoding runs in your browser, the string is never sent to our servers before you use it.' },
    ],
  ...
  'base64-decoder': {
    ...
    faqs: [
      ...
      { question: 'How do I convert Base64 back to text?', answer: 'Paste the Base64 string into the input field on this decoder, then click Decode. The tool reverses the encoding and shows the original plain text in the output. Copy the result when you are done. It works for standard Base64 and URL-safe Base64URL strings commonly found in JWTs.' },
      { question: 'Can I decode Base64 to text without installing software?', answer: 'Yes. This decoder runs entirely in your browser, so you do not need Python, OpenSSL, or any local package. Open the page, paste the encoded string, and decode it client-side. Your data stays on your device and is never uploaded for processing.' },
      { question: 'Is \'decode Base64\' the same as \'convert Base64 to text\'?', answer: 'Yes. Decode Base64 and convert Base64 to text are the same operation described with different wording. Both mean reversing Base64 encoding to recover the original text. This page does that conversion: paste an encoded string, decode it, and read or copy the plain-text output.' },
      { question: 'How do I convert a Base64 string to readable text online?', answer: 'Use this online decoder: paste the full Base64 value (including any = padding), click Decode, and review the readable text in the result field. If the output looks like gibberish, the original data was likely binary rather than text — in that case a specialised binary-aware tool is a better fit.' },
    ],
  ...
  'jwt-decoder': {
    ...
    faqs: [
      ...
      { question: 'What is a JSON Web Signature (JWS)?', answer: 'A JSON Web Signature (JWS) is the signed structure behind most JWTs: three Base64URL segments joined by dots — header.payload.signature. In practice, a compact JWT is a JWS that carries JSON claims in the payload. The terms are often used interchangeably. This tool decodes both the JWT claims and the underlying JWS segments so you can inspect header, payload, and signature bytes.' },
      { question: 'Can I decode a JWT online without a secret key?', answer: 'Yes. The header and payload are Base64URL encoded, not encrypted, so they can always be decoded without a secret or public key. Only signature verification needs the signing secret or public key. This online JWT decoder reads the encoded parts for inspection and does not require a key for basic decoding.' },
      { question: 'What\'s the difference between a JWT decoder and a JWT verifier?', answer: 'A JWT decoder Base64URL-decodes the header and payload so you can read claims and metadata. A JWT verifier goes further: it checks the signature against a secret or public key to confirm the token was issued by the expected party and has not been altered. This tool is a decoder — it shows contents without performing cryptographic verification.' },
      { question: 'Is it safe to decode a JWT online?', answer: 'With this tool, decoding is client-side only: your JWT token is never sent to any server. That said, treat production tokens as credentials and avoid pasting them into untrusted sites. For high-sensitivity debugging, prefer local tools or your browser developer tools even when a page processes data in-browser.' },
    ],
  ...
  'webp-to-png': {
    ...
    faqs: [
      ...
      { question: 'Why would I convert WebP to PNG instead of keeping WebP?', answer: 'Keep WebP for modern web delivery when size matters. Convert to PNG when a tool or workflow cannot open WebP — older image editors, some desktop apps, print pipelines, or platforms that reject WebP uploads. PNG is also a common choice when you need to edit transparency in software that expects PNG, or when sharing assets with collaborators who do not have WebP support.' },
      { question: 'Does converting WebP to PNG preserve transparency?', answer: 'Yes, when the source WebP uses an alpha channel. Both WebP and PNG support transparency, and this converter preserves alpha during conversion. If the WebP has no alpha (fully opaque), the PNG will also be opaque — there is no transparency to preserve in that case.' },
      { question: 'Can I convert multiple WebP files to PNG at once?', answer: 'Not in the current version. The converter accepts one WebP file at a time: upload, convert, then download the PNG. For a batch of images, repeat the process for each file, or use a local script if you need bulk conversion regularly.' },
      { question: 'Is this WebP to PNG converter free and unlimited?', answer: 'Yes. The tool is free to use with no signup, no account, and no watermark on the downloaded PNG. Conversion runs in your browser, so there is no per-file upload quota from our servers — you can convert as many images as you need, one at a time.' },
      { question: 'How do I convert WebP to PNG online?', answer: 'Upload your .webp file on this page. The tool converts it to PNG in your browser automatically, then you can download the result. No software install and no server upload — useful when you need a quick online WebP to PNG conversion for compatibility with editors or platforms that do not accept WebP.' },
    ],
    trustNote: 'All image processing runs in your browser — your images are never uploaded to any server.',
  },
  ...
  'chmod-calculator': {
    howToUse: {
      steps: [
        'Click permission checkboxes for owner, group, and others, or use a preset like 644 or 755.',
        'Type an octal mode (755 or 4755) or symbolic string (rwxr-xr-x) — all inputs stay in sync.',
        'Review octal, symbolic, and chmod command outputs below the grid.',
        'Copy the numeric or symbolic chmod command for your deploy script or server session.',
      ],
    },
    expandedDescription: [
    ...
    useCases: [
      { title: 'Deploy script permissions', description: 'Confirm deploy.sh is 755 before chmod +x on the server.' },
      { title: 'SSH key hygiene', description: 'Set id_rsa to 600 after copying to ~/.ssh to avoid OpenSSH warnings.' },
      { title: 'Shared log directories', description: 'Use setgid on a log folder so new files inherit the correct group.' },
      { title: 'Read-only secrets', description: 'Apply 400 to files that should never be modified by the app user.' },
      { title: 'Learning chmod notation', description: 'Map between rwx grids and the numeric modes you see in ls -l.' },
      { title: 'Documenting runbooks', description: 'Copy symbolic chmod u=rwx,g=rx,o=rx commands into infrastructure docs.' },
    ],
    faqs: [
      { question: 'How do I read chmod 755?', answer: '7 = rwx for owner, 5 = r-x for group, 5 = r-x for others. It is the standard mode for directories and executable scripts.' },
      { question: 'When should I use 644 vs 755?', answer: 'Use 644 for normal files that should not be executed. Use 755 for directories and anything that must run as a script or binary.' },
      { question: 'Why does SSH reject my private key?', answer: 'Keys must not be readable by group or others. Use chmod 600 on id_rsa and chmod 700 on ~/.ssh.' },
      { question: 'What is the sticky bit on /tmp?', answer: 'chmod 1777 on /tmp sets sticky so users can create files but only delete their own, even when the directory is world-writable.' },
      { question: 'What does setuid do?', answer: 'When set on an executable, the process runs with the file owner’s privileges — for example passwd running as root.' },
      { question: 'Can I use four-digit octal?', answer: 'Yes. The leading digit encodes setuid (4), setgid (2), and sticky (1), e.g. 4755 for setuid plus 755 permissions.' },
      { question: 'Does execute on a directory mean run?', answer: 'No. Execute on a directory grants traverse access — you need x to cd into it or access paths beneath it.' },
      { question: 'Is my input uploaded?', answer: 'No. Permission math runs entirely in your browser.' },
    ],
    trustNote: 'Permission calculations run locally in your browser — nothing is sent to a server.',
  },

  'px-to-rem-converter': {
    howToUse: {
      steps: [
        'Set the root font size (default 16px) to match your html { font-size }.',
        'Enter a value in px or rem — the other field updates automatically.',
        'Paste CSS into batch mode and click Convert CSS to replace px lengths with rem.',
        'Use Skip 1px and Skip media queries when converting production stylesheets.',
      ],
    },
    expandedDescription: [
    ...
    useCases: [
      { title: 'Design handoff', description: 'Convert Figma px specs to rem for a 16px or 10px root design system.' },
      { title: 'Accessibility audits', description: 'Move typography and spacing to rem so user font settings scale the UI.' },
      { title: 'Legacy CSS cleanup', description: 'Batch-convert an old px stylesheet while skipping borders and media queries.' },
      { title: 'Component libraries', description: 'Build a reference table of common spacing tokens at your chosen base.' },
      { title: 'Tailwind migration', description: 'Translate arbitrary px values from prototypes into rem utilities.' },
      { title: 'Documentation', description: 'Show designers the rem equivalent of standard pixel steps in your style guide.' },
    ],
    faqs: [
      { question: 'What is the default rem base?', answer: 'Most browsers use 16px on html, so 1rem = 16px unless your project changes the root font size.' },
      { question: 'What is the 62.5% trick?', answer: 'Setting html { font-size: 62.5% } makes 1rem = 10px when the browser default is 16px. You must reset body text size and watch for broken third-party CSS.' },
      { question: 'rem vs em?', answer: 'rem is always relative to the root. em is relative to the parent font size and stacks when nested.' },
      { question: 'Why skip 1px in batch mode?', answer: '1px borders and outlines often stay in px so they remain crisp; converting them to rem can produce sub-pixel rendering.' },
      { question: 'Why skip media queries?', answer: 'Breakpoints are frequently kept in px to match device pixels; skipping @media blocks avoids changing those thresholds.' },
      { question: 'How many decimals are used?', answer: 'Values are rounded to four decimal places and trailing zeros are stripped (e.g. 1.5rem).' },
      { question: 'Does batch mode upload CSS?', answer: 'No. Conversion runs locally in your browser.' },
      { question: 'Can I download converted CSS?', answer: 'Yes. Use Download to save converted.css after batch conversion.' },
    ],
    trustNote: 'All px/rem math runs in your browser — your CSS is never uploaded.',
  },

  'env-to-json-converter': {
    howToUse: {
      steps: [
        'Paste a .env file or flat JSON object into the input area.',
        'Check the direction badge (.env → JSON or JSON → .env) and click Swap direction if needed.',
        'Review warnings for duplicate keys, invalid names, or lines that could not be parsed.',
        'Copy or download the output; enable Mask values when presenting on a shared screen.',
      ],
    },
    expandedDescription: [
    ...
    useCases: [
      { title: 'Vercel / Netlify env import', description: 'Convert JSON from a secret manager into paste-ready .env lines.' },
      { title: 'Docker debugging', description: 'Compare .env parsing with what docker run --env-file actually loads.' },
      { title: 'CI variable setup', description: 'Turn flat JSON from GitHub Actions secrets into .env format for local testing.' },
      { title: 'Onboarding docs', description: 'Generate .env.example from a JSON template without real values.' },
      { title: 'Config migration', description: 'Move from dotenv files to JSON configs for serverless platforms.' },
      { title: 'Screen-shared demos', description: 'Mask values while showing structure to teammates.' },
    ],
    faqs: [
      { question: 'How is direction detected?', answer: 'If the trimmed input starts with {, it is treated as JSON → .env. Otherwise it is parsed as .env → JSON.' },
      { question: 'What happens with duplicate keys?', answer: 'The last value wins. Warnings list each line number where the key appeared.' },
      { question: 'Are nested JSON objects supported?', answer: 'No. Nested objects and arrays return an error that names the key that must be flattened.' },
      { question: 'Does Docker strip quotes?', answer: 'docker run --env-file typically does not strip quotes; Compose v2 handling often does. Always verify in your target environment.' },
      { question: 'What keys are valid?', answer: 'This tool warns when keys do not match ^[A-Za-z_][A-Za-z0-9_]*$, which matches common dotenv conventions.' },
      { question: 'Does mask affect downloads?', answer: 'Mask replaces values with •••• in the display and in copy/download when enabled — use it only when you intend to hide secrets.' },
      { question: 'Is export supported?', answer: 'Yes. A leading export on a line is stripped before parsing KEY=value.' },
      { question: 'Is my .env uploaded?', answer: 'No. Parsing runs entirely in your browser.' },
    ],
    trustNote: 'Environment data is parsed locally — secrets never leave your device.',
  },

  'line-ending-converter': {
    howToUse: {
      steps: [
        'Paste text or upload a file — the detector counts CRLF, LF-only, and CR-only lines.',
        'Choose LF (Unix/macOS), CRLF (Windows), or CR (classic Mac) as the target.',
        'Enable Remove BOM (default) and optionally ensure a trailing newline.',
        'Click Convert, preview the text, then Download — uploaded files keep their original name.',
      ],
    },
    expandedDescription: [
    ...
    useCases: [
      { title: 'Fix Windows-edited .sh scripts', description: 'Convert CRLF to LF before deploying to Linux containers.' },
      { title: 'Normalize repo files', description: 'Clean a file before adding .gitattributes rules.' },
      { title: 'CSV from Excel', description: 'Remove BOM and switch to LF for Unix-friendly imports.' },
      { title: 'CI lint failures', description: 'Resolve “expected LF” errors from eslint or editorconfig checks.' },
      { title: 'Legacy Mac CR files', description: 'Migrate ancient CR-only text to LF or CRLF.' },
      { title: 'Cross-team handoffs', description: 'Send Windows teammates CRLF while keeping LF in git.' },
    ],
    faqs: [
      { question: 'What is CRLF vs LF?', answer: 'CRLF is carriage return + line feed (\\r\\n), used on Windows. LF is line feed only (\\n), used on Unix and macOS.' },
      { question: 'Why does bash show ^M?', answer: 'The CR character remains in the file. Convert to LF so the shebang line is #!/bin/bash with no extra carriage return.' },
      { question: 'What is a UTF-8 BOM?', answer: 'An invisible U+FEFF prefix at the start of a UTF-8 file. It can break scripts and parsers; this tool can strip it on convert.' },
      { question: 'Why does git show every line changed?', answer: 'Mixed or flipped line endings change bytes on every line. Normalize endings to fix noisy diffs.' },
      { question: 'Can I see endings in the preview?', answer: 'No — line endings are invisible in the textarea. Trust the download for exact bytes.' },
      { question: 'What filename is used?', answer: 'Uploaded files keep their name; pasted text downloads as converted.txt.' },
      { question: 'How is conversion done?', answer: 'All input is normalized to LF internally, then rewritten as LF, CRLF, or CR.' },
      { question: 'Is file content uploaded?', answer: 'No. FileReader and conversion run locally in your browser.' },
    ],
    trustNote: 'Files are read and converted in your browser — nothing is uploaded to a server.',
  },

  'nanoid-generator': {
    howToUse: {
      steps: [
        'Choose NanoID or ULID mode at the top of the tool.',
        'For NanoID, set length (default 21), alphabet preset, and count (1–100).',
        'For ULID, set count — each ID is 26 Crockford base32 characters with a decoded ISO timestamp.',
        'Click Generate, then copy individual IDs or Copy All for your app or database seed.',
      ],
    },
    expandedDescription: [
    ...
    useCases: [
      { title: 'URL slugs and public IDs', description: 'Issue short NanoIDs for share links instead of sequential integers.' },
      { title: 'Sortable primary keys', description: 'Use ULIDs when you want time-ordered inserts without a separate created_at index.' },
      { title: 'Log correlation', description: 'Generate batches of IDs for tracing requests across services.' },
      { title: 'Test fixtures', description: 'Seed databases with unique IDs during local development.' },
      { title: 'Session tokens (with care)', description: 'Prototype token formats — always pair production tokens with server-side validation.' },
      { title: 'Comparing ID formats', description: 'Evaluate NanoID length vs UUID v4 string size in API payloads.' },
    ],
    faqs: [
      { question: 'NanoID vs UUID v4?', answer: 'NanoID is shorter (21 vs 36 characters), URL-safe by default, and offers similar collision resistance at default settings.' },
      { question: 'What is a ULID?', answer: 'A 26-character lexicographically sortable ID: timestamp plus randomness in Crockford base32.' },
      { question: 'Why are ULIDs sortable?', answer: 'The time component is at the start, so string sort order matches creation order within the same millisecond granularity.' },
      { question: 'What is UUID v7?', answer: 'A UUID standard with a Unix timestamp in the high bits — another time-ordered option alongside ULID.' },
      { question: 'Does shorter NanoID risk collisions?', answer: 'Yes. Fewer characters mean less entropy. Estimate volume before using very short IDs in production.' },
      { question: 'Is Math.random() used?', answer: 'No. This tool uses crypto.getRandomValues for secure random bytes.' },
      { question: 'What alphabets are available?', answer: 'Default URL-safe, alphanumeric, lowercase, numbers only, or a custom alphabet with rejection sampling to avoid modulo bias.' },
      { question: 'How many IDs per click?', answer: 'You can generate between 1 and 100 IDs at once.' },
    ],
    trustNote: 'IDs are generated locally with crypto.getRandomValues — nothing is sent to a server.',
  },
};
;
