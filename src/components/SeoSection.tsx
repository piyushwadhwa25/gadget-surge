import { Link } from 'react-router-dom';
import type { ToolConfig } from '@/lib/tools-registry';
import { useCaseLabels } from '@/lib/tools-registry';
import type { ToolContent } from '@/lib/toolContentMap';
import { Shield } from 'lucide-react';

interface SeoSectionProps {
  tool: ToolConfig;
  expanded?: ToolContent;
}

export function SeoSection({ tool, expanded }: SeoSectionProps) {
  if (expanded) {
    return (
      <div className="mt-12 space-y-10">
        <section>
          <h2 className="text-xl font-bold text-foreground mb-3">How to Use</h2>
          <ol className="list-decimal list-outside pl-5 space-y-2 text-muted-foreground leading-relaxed max-w-prose">
            {expanded.howToUse.steps.map((step, i) => (
              <li key={i}>{step}</li>
            ))}
          </ol>
        </section>

        <section>
          <h2 className="text-xl font-bold text-foreground mb-3">About {tool.name}</h2>
          {expanded.expandedDescription.map((para, i) => (
            <p key={i} className="text-muted-foreground leading-relaxed max-w-prose mb-4 last:mb-0">
              {para}
            </p>
          ))}
          <p className="text-sm text-muted-foreground mt-3 max-w-prose">
            This tool is part of our{' '}
            <Link to={`/category/${tool.categorySlug}`} className="text-primary hover:underline">
              {tool.category}
            </Link>{' '}
            collection.
          </p>
        </section>

        <section>
          <h2 className="text-xl font-bold text-foreground mb-4">Example</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="overflow-hidden rounded-lg border border-border">
              <div className="px-4 py-2 bg-muted/60 border-b border-border">
                <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Input</h3>
              </div>
              <pre className="font-code text-xs p-4 overflow-x-auto text-foreground whitespace-pre-wrap break-all bg-card">
                {tool.exampleInput}
              </pre>
            </div>
            <div className="overflow-hidden rounded-lg border border-border">
              <div className="px-4 py-2 bg-muted/60 border-b border-border">
                <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Output</h3>
              </div>
              <pre className="font-code text-xs p-4 overflow-x-auto text-foreground whitespace-pre-wrap break-all bg-card">
                {tool.exampleOutput}
              </pre>
            </div>
          </div>
        </section>

        <section>
          <h2 className="text-xl font-bold text-foreground mb-4">Common Use Cases</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {expanded.useCases.map((uc, i) => (
              <div
                key={i}
                className="rounded-lg border border-border bg-card/50 p-4 text-sm text-muted-foreground leading-relaxed"
              >
                <p className="font-semibold text-foreground mb-1">{uc.title}</p>
                <p>{uc.description}</p>
              </div>
            ))}
          </div>
        </section>

        {tool.useCaseTags && tool.useCaseTags.length > 0 && (
          <section>
            <h3 className="text-sm font-medium text-foreground mb-2">Browse by use case:</h3>
            <div className="flex flex-wrap gap-2">
              {tool.useCaseTags.map(tag => (
                <Link
                  key={tag}
                  to={`/tools?useCase=${tag}`}
                  className="px-3 py-1 text-xs font-medium rounded-full border border-input bg-background text-muted-foreground hover:bg-muted hover:text-foreground transition-colors"
                >
                  {useCaseLabels[tag]}
                </Link>
              ))}
            </div>
          </section>
        )}

        <section>
          <h2 className="text-xl font-bold text-foreground mb-4">Frequently Asked Questions</h2>
          <dl className="space-y-6">
            {expanded.faqs.map((faq, i) => (
              <div key={i} className="border-b border-border pb-6 last:border-0 last:pb-0">
                <dt className="font-semibold text-foreground mb-2 leading-snug">{faq.question}</dt>
                <dd className="text-muted-foreground leading-relaxed">{faq.answer}</dd>
              </div>
            ))}
          </dl>
        </section>

        <p className="flex items-start gap-2 text-sm text-muted-foreground max-w-prose">
          <Shield className="h-4 w-4 shrink-0 mt-0.5 text-primary" aria-hidden />
          <span>
            <span className="font-medium text-foreground">Why use GadgetSurge? </span>
            {expanded.trustNote} No signup required — everything runs in your browser.
          </span>
        </p>
      </div>
    );
  }

  return (
    <div className="mt-12 space-y-10">
      <section>
        <h2 className="text-xl font-bold text-foreground mb-3">What This Tool Does</h2>
        <p className="text-muted-foreground leading-relaxed max-w-prose">{tool.introText}</p>
        <p className="text-sm text-muted-foreground mt-3 max-w-prose">
          This tool is part of our{' '}
          <Link to={`/category/${tool.categorySlug}`} className="text-primary hover:underline">
            {tool.category}
          </Link>{' '}
          collection. All processing happens in your browser — your data never leaves your device.
        </p>
      </section>

      <section>
        <h2 className="text-xl font-bold text-foreground mb-4">Example</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="overflow-hidden rounded-lg border border-border">
            <div className="px-4 py-2 bg-muted/60 border-b border-border">
              <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Input</h3>
            </div>
            <pre className="font-code text-xs p-4 overflow-x-auto text-foreground whitespace-pre-wrap break-all bg-card">
              {tool.exampleInput}
            </pre>
          </div>
          <div className="overflow-hidden rounded-lg border border-border">
            <div className="px-4 py-2 bg-muted/60 border-b border-border">
              <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Output</h3>
            </div>
            <pre className="font-code text-xs p-4 overflow-x-auto text-foreground whitespace-pre-wrap break-all bg-card">
              {tool.exampleOutput}
            </pre>
          </div>
        </div>
      </section>

      {tool.useCases.length > 0 && (
        <section>
          <h2 className="text-xl font-bold text-foreground mb-3">Common Use Cases</h2>
          <ul className="space-y-2 text-muted-foreground">
            {tool.useCases.map((uc, i) => (
              <li key={i} className="flex items-start gap-2">
                <span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-primary shrink-0" />
                <span>{uc}</span>
              </li>
            ))}
          </ul>
        </section>
      )}

      {tool.useCaseTags && tool.useCaseTags.length > 0 && (
        <section>
          <h3 className="text-sm font-medium text-foreground mb-2">Browse by use case:</h3>
          <div className="flex flex-wrap gap-2">
            {tool.useCaseTags.map(tag => (
              <Link
                key={tag}
                to={`/tools?useCase=${tag}`}
                className="px-3 py-1 text-xs font-medium rounded-full border border-input bg-background text-muted-foreground hover:bg-muted hover:text-foreground transition-colors"
              >
                {useCaseLabels[tag]}
              </Link>
            ))}
          </div>
        </section>
      )}

      {tool.faqItems.length > 0 && (
        <section>
          <h2 className="text-xl font-bold text-foreground mb-4">Frequently Asked Questions</h2>
          <dl className="space-y-6">
            {tool.faqItems.map((faq, i) => (
              <div key={i} className="border-b border-border pb-6 last:border-0 last:pb-0">
                <dt className="font-semibold text-foreground mb-2 leading-snug">{faq.question}</dt>
                <dd className="text-muted-foreground leading-relaxed">{faq.answer}</dd>
              </div>
            ))}
          </dl>
        </section>
      )}
    </div>
  );
}
