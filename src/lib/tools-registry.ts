...
export const categories: CategoryConfig[] = [
  {
    ...
    introText: "GadgetSurge's developer tools handle the plumbing work every backend and frontend engineer runs into daily — formatting, encoding, and validating data before it ships. Every tool processes data in your browser tab; nothing you paste is ever sent to a server.",
    whoIsItFor: 'Backend engineers, frontend developers, and anyone debugging an API response, a malformed JSON payload, or a broken JWT.',
    commonUseCases: ['Formatting and validating JSON payloads before committing a fix', 'Decoding a JWT to inspect its claims without pasting it into a third-party site', 'Converting between CSV exports and JSON for scripts', 'Generating UUIDs for test fixtures or database seeds'],
    faqItems: [
      { question: 'Is it safe to paste API keys, tokens, or JWTs into these tools?', answer: "Yes — every tool on this page runs entirely in your browser using JavaScript. Nothing you type or paste is transmitted anywhere, logged, or stored. You can verify this yourself by opening your browser's network tab while using any tool here." },
      { question: 'Do these tools work offline?', answer: 'Once the page has loaded, most of them do — formatting, encoding, and validation logic runs locally. Processing your data does not require an internet connection.' },
      { question: "What's the difference between the JSON formatter and JSON-to-CSV converter?", answer: "The formatter reorganizes existing JSON into a readable structure; the converter transforms JSON's nested structure into flat tabular rows, which only works cleanly on relatively flat data." },
    ],
  ...
  },
  {
    ...
    introText: "Every tool below resizes, converts, or compresses images directly in your browser using the Canvas API — your photos are never uploaded to a server, which matters if you're working with anything you'd rather not send over the wire.",
    whoIsItFor: "Anyone who needs to quickly resize, convert, or compress an image without installing software or trusting a third-party upload service.",
    commonUseCases: ['Converting PNG screenshots to WebP for faster-loading web pages', 'Compressing photos before attaching them to an email', 'Cropping and resizing images to fit exact dimensions for social posts', "Extracting a dominant color or checking an image's exact pixel dimensions"],
    faqItems: [
      { question: 'Why choose WebP over JPG?', answer: "WebP typically produces 25-35% smaller files than JPG at equivalent visual quality, and it supports transparency, which JPG doesn't. The tradeoff is that some older software and email clients still don't render WebP correctly — JPG remains the safer default for maximum compatibility." },
      { question: 'Are my images uploaded anywhere?', answer: 'No. All processing happens locally in your browser tab using the Canvas API. Your images never leave your device.' },
      { question: 'What resolution or quality settings should I use?', answer: "For web use, 75-85% JPG/WebP quality and image widths matched to your actual display size, not your camera's full resolution, keep files small without visible quality loss." },
    ],
  ...
  },
  {
    ...
    introText: "Small, focused text utilities — counting, converting case, cleaning up whitespace, generating passwords — for the kind of quick fix you don't want to open a full editor for.",
    whoIsItFor: 'Writers, students, and developers doing a quick text transformation.',
    commonUseCases: ['Checking word or character counts against a submission limit', 'Cleaning up pasted text with inconsistent spacing or line breaks', 'Generating a random password or username for a new account'],
    faqItems: [
      { question: 'Do these tools save or store the text I enter?', answer: 'No — text is processed in memory in your browser and cleared when you leave the page or refresh.' },
      { question: 'Can I process very long documents?', answer: 'Yes, most tools handle documents up to several hundred thousand characters without slowdown, since everything runs locally rather than over a network request.' },
    ],
  ...
  },
  {
    name: 'Document Tools',
    slug: 'document-tools',
    description: 'Tools for PDF editing, text analysis, and document processing.',
    introText: "PDF and document tools that keep your files on your device the entire time — no upload, no third-party server processing your contracts, IDs, or scanned paperwork.",
    whoIsItFor: "Anyone merging, splitting, or trimming PDFs who'd rather not upload a sensitive document to an unfamiliar website.",
    commonUseCases: ['Merging multiple scanned pages into a single PDF before submitting an application', 'Removing a signature page or blank page from a scanned document', 'Splitting a large PDF into individual chapters or sections', 'Counting words in a document draft before submission'],
    faqItems: [
      { question: 'Is it safe to process personal documents like IDs or contracts here?', answer: 'Yes — PDF merging, splitting, and page removal happen entirely in your browser using client-side libraries. Your files are never uploaded to a server, which is worth checking on any tool handling sensitive documents, not just this one.' },
      { question: 'Is there a file size limit?', answer: "Very large PDFs (100+ MB) may be slow since your device's memory is doing the processing rather than a server's. Most everyday documents process in a few seconds." },
      { question: 'Will merging or splitting affect PDF quality?', answer: "No — these operations rearrange existing pages rather than re-rendering them, so there's no quality loss or recompression involved." },
    ],
  ...
  },
  {
    name: 'Calculators',
    slug: 'calculators',
    description: 'Developer-focused calculators for timestamps, cron, contrast, and data sizes.',
    introText: "A small set of developer-adjacent calculators that don't fit neatly under text, image, or document tools — timestamp conversion, cron syntax, contrast ratios, and data size math, grouped here because you'll often need more than one of them in the same debugging session.",
    whoIsItFor: 'Developers scheduling cron jobs, checking accessibility contrast ratios, or converting between storage units and Unix timestamps.',
    commonUseCases: ['Converting a Unix timestamp to a human-readable date while debugging logs', 'Validating a cron expression before deploying a scheduled job', 'Checking whether a text/background color pair meets WCAG contrast requirements'],
    faqItems: [
      { question: "What's the difference between MB and MiB in the data size converter?", answer: "MB (megabyte) uses base-10 — 1 MB = 1,000,000 bytes — the convention storage manufacturers use. MiB (mebibyte) uses base-2 — 1 MiB = 1,048,576 bytes — the convention operating systems typically use to report file sizes. This is why a '500 GB' drive shows as roughly 465 GiB in your OS." },
      { question: 'What WCAG contrast ratio should I target?', answer: 'WCAG AA requires a 4.5:1 ratio for normal text and 3:1 for large text (18pt+ or 14pt+ bold). WCAG AAA raises that to 7:1 and 4.5:1 respectively.' },
      { question: 'Does the cron expression calculator support non-standard syntax like @daily?', answer: "Standard 5-field cron syntax is fully supported; check the tool page itself for which shorthand expressions are recognized, since support varies by the underlying cron implementation you're targeting." },
    ],
    relatedCategorySlugs: ['developer-tools', 'image-tools'],
  },
];
...
export const tools: ToolConfig[] = [
  ...
  },
  {
    ...
    relatedToolSlugs: ['timestamp-converter', 'json-formatter', 'base64-encoder', 'nanoid-generator'],
    type: 'custom',
  ...
  },
  {
    ...
    category: 'Developer Tools',
    categorySlug: 'developer-tools',
    description: 'Calculate Unix file permissions in octal and symbolic form with chmod commands.',
    keywords: ['chmod calculator', 'chmod 755', 'linux file permissions', 'unix permissions calculator'],
    seoTitle: 'chmod Calculator — Octal & Symbolic Permissions | GadgetSurge',
    metaDescription: 'Convert between chmod octal (755) and symbolic (rwxr-xr-x) permissions. Copy chmod commands instantly. Runs in your browser — no upload.',
    introText: 'Toggle read, write, and execute for owner, group, and others — or type octal or symbolic modes. Get octal, symbolic, and ready-to-run chmod commands, including setuid, setgid, and sticky bits.',
    exampleInput: '755',
    exampleOutput: 'Octal: 755\nSymbolic: rwxr-xr-x\nCommand: chmod 755 filename',
    faqItems: [
      { question: 'What does chmod 755 mean?', answer: 'Owner can read, write, and execute (7); group and others can read and execute (5). Common for scripts and directories.' },
      { question: 'Why is 777 dangerous?', answer: 'Mode 777 lets anyone read, write, and execute the file. It is a common but unsafe fix for permission errors on web servers.' },
      { question: 'What are setuid and sticky bits?', answer: 'setuid (4xxx) runs a program as the file owner; setgid (2xxx) inherits group; sticky (1xxx on dirs) restricts deletion in /tmp to the file owner.' },
    ],
    relatedToolSlugs: ['cron-expression-calculator', 'env-to-json-converter', 'line-ending-converter'],
    type: 'custom',
    useCases: ['Setting deploy script permissions', 'Fixing SSH key modes', 'Documenting server file ACLs'],
    recentlyAdded: true,
    useCaseTags: ['debugging', 'productivity'],
  },
  {
    ...
    category: 'Developer Tools',
    categorySlug: 'developer-tools',
    description: 'Convert px to rem and rem to px with a configurable root font size and CSS batch mode.',
    keywords: ['px to rem', 'rem calculator', 'css units converter', 'px to em'],
    seoTitle: 'PX to REM Converter — CSS Unit Calculator | GadgetSurge',
    metaDescription: 'Convert px to rem with a custom root font size. Batch-convert CSS and skip 1px borders. Free converter runs entirely in your browser.',
    introText: 'Link px and rem fields at your chosen base (default 16px), batch-convert pasted CSS, and use a live reference table for common pixel sizes.',
    exampleInput: '16px base, 24px',
    exampleOutput: '1.5rem',
    faqItems: [
      { question: 'What is the default rem base?', answer: 'Browsers default to 16px on the root html element, so 1rem = 16px unless you change it.' },
      { question: 'Should borders stay in px?', answer: 'Many teams keep 1px borders in px so hairlines stay crisp; this tool can skip 1px values in batch mode.' },
      { question: 'Does this upload my CSS?', answer: 'No. All conversion runs locally in your browser.' },
    ],
    relatedToolSlugs: ['color-converter', 'color-contrast-checker', 'html-formatter'],
    type: 'custom',
    useCases: ['Migrating designs to rem', 'Accessibility-friendly sizing', 'Cleaning legacy px CSS'],
    recentlyAdded: true,
    useCaseTags: ['conversion'],
  },
  {
    ...
    category: 'Developer Tools',
    categorySlug: 'developer-tools',
    description: 'Convert .env files to JSON and JSON to .env with dotenv-style parsing.',
    keywords: ['env to json', 'json to env', 'dotenv converter', 'environment variables'],
    seoTitle: 'ENV to JSON Converter — Dotenv ↔ JSON | GadgetSurge',
    metaDescription: 'Convert .env to JSON or JSON to .env with export stripping and quote rules. Warnings for duplicates. Runs in your browser — secrets stay local.',
    introText: 'Paste a .env file or flat JSON object. Auto-detect direction, swap manually, and see warnings for duplicate keys, invalid key names, and unparseable lines.',
    exampleInput: 'API_KEY=abc\nDB_HOST="localhost"',
    exampleOutput: '{\n  "API_KEY": "abc",\n  "DB_HOST": "localhost"\n}',
    faqItems: [
      { question: 'Are nested JSON objects supported?', answer: 'No. Only flat key-value objects are supported; nested keys return a clear error.' },
      { question: 'How are quotes handled?', answer: 'Matching single or double quotes are stripped; double-quoted values expand \\n to newlines.' },
      { question: 'Can I mask values for screen sharing?', answer: 'Yes. Enable mask values to show •••• in the output and copied text.' },
    ],
    relatedToolSlugs: ['json-formatter', 'chmod-calculator', 'base64-encoder'],
    type: 'custom',
    useCases: ['Preparing Vercel env blocks', 'Migrating secrets to JSON', 'Debugging docker --env-file'],
    recentlyAdded: true,
    useCaseTags: ['conversion', 'debugging'],
  },
  {
    ...
    category: 'Developer Tools',
    categorySlug: 'developer-tools',
    description: 'Detect and convert CRLF, LF, and CR line endings with optional BOM removal.',
    keywords: ['crlf to lf', 'line ending converter', 'convert crlf', 'windows to unix line endings'],
    seoTitle: 'Line Ending Converter — CRLF to LF Online | GadgetSurge',
    metaDescription: 'Convert CRLF, LF, or CR line endings. Detect mixed endings and UTF-8 BOM. Download fixed files — processing runs entirely in your browser.',
    introText: 'Paste text or upload a file, see CRLF/LF/CR counts, convert to your target ending, remove BOM, and download with the original filename when uploaded.',
    exampleInput: 'Line one\r\nLine two',
    exampleOutput: 'LF-converted text ready to download',
    faqItems: [
      { question: 'Why do shell scripts fail with ^M?', answer: 'Windows CRLF leaves a carriage return before the newline, breaking the shebang: /bin/bash^M: bad interpreter.' },
      { question: 'Can I keep the uploaded filename?', answer: 'Yes. Downloads use the uploaded filename; pasted text saves as converted.txt.' },
      { question: 'Is the preview exact?', answer: 'Line endings are invisible in the preview; use Download for the converted bytes.' },
    ],
    relatedToolSlugs: ['remove-line-breaks', 'env-to-json-converter', 'chmod-calculator'],
    type: 'custom',
    useCases: ['Fixing .sh scripts from Windows', 'Normalizing repo line endings', 'Removing UTF-8 BOM from configs'],
    recentlyAdded: true,
    useCaseTags: ['conversion', 'text-cleanup'],
  },
  {
    ...
    category: 'Developer Tools',
    categorySlug: 'developer-tools',
    description: 'Generate NanoID and monotonic ULID identifiers with crypto.getRandomValues.',
    keywords: ['nanoid generator', 'ulid generator', 'short unique id', 'random id generator'],
    seoTitle: 'NanoID Generator — Short IDs & ULID Online | GadgetSurge',
    metaDescription: 'Generate URL-safe NanoIDs and sortable ULIDs with crypto.getRandomValues. Bulk generation up to 100. Runs entirely in your browser.',
    introText: 'Create NanoIDs with custom length and alphabet, or Crockford base32 ULIDs with decoded ISO timestamps. Uses crypto.getRandomValues — not Math.random().',
    exampleInput: 'NanoID length 21, count 5',
    exampleOutput: 'Five unique IDs, one per line',
    faqItems: [
      { question: 'NanoID vs UUID v4?', answer: 'Default 21-char NanoID is shorter and URL-safe with similar collision resistance to UUID v4.' },
      { question: 'What is a ULID?', answer: 'A 26-character time-sortable ID: 48-bit timestamp plus 80 bits of randomness in Crockford base32.' },
      { question: 'Is randomness secure?', answer: 'Yes. This tool uses crypto.getRandomValues for NanoID and ULID random portions.' },
    ],
    relatedToolSlugs: ['uuid-generator', 'random-password-generator', 'unix-timestamp-converter'],
    type: 'custom',
    useCases: ['Database primary keys', 'URL-safe tokens', 'Sortable log correlation IDs'],
    recentlyAdded: true,
    useCaseTags: ['generators'],
  },
  // ===== TEXT TOOLS =====
  ...
  // ===== DOCUMENT TOOLS =====
  {
    ...
    seoTitle: 'PDF Merger Online — Combine PDF Files Free',
    metaDescription: 'Merge multiple PDF files into one document instantly. Reorder files before merging. Free online PDF merger — runs in your browser.',
    introText: 'Upload multiple PDF files, arrange them in the order you want, and download a single merged PDF. All processing happens in your browser — your documents never leave your device.',
    exampleInput: 'Upload: report-part1.pdf, report-part2.pdf, appendix.pdf',
    exampleOutput: 'merged.pdf (combined in upload order)',
    faqItems: [
      { question: 'How many PDFs can I merge?', answer: 'There is no fixed limit. You can merge as many PDFs as your browser can handle in memory.' },
      { question: 'Can I change the merge order?', answer: 'Yes. Use the up and down arrows to reorder files before clicking Merge.' },
      { question: 'Are my PDFs uploaded to a server?', answer: 'No. Merging happens entirely in your browser using pdf-lib.' },
      { question: 'What if a PDF is password-protected?', answer: 'Encrypted or password-protected PDFs cannot be merged. Remove the password first using your PDF reader.' },
    ],
    relatedToolSlugs: ['pdf-splitter', 'pdf-page-remover', 'word-counter'],
    type: 'custom',
    useCases: ['Combining report sections', 'Merging scanned documents', 'Joining invoice batches'],
    featured: true,
    popular: true,
    recentlyAdded: true,
    useCaseTags: ['conversion', 'productivity'],
  },
  {
    ...
    seoTitle: 'PDF Page Remover Online — Delete PDF Pages Free',
    metaDescription: 'Remove unwanted pages from a PDF instantly. Select pages by number or range. Free online PDF page remover — 100% client-side.',
    introText: 'Upload a PDF, select pages to remove using checkboxes or page numbers like "2, 5, 7-9", and download the edited PDF with those pages removed.',
    exampleInput: 'Upload: document.pdf (10 pages)\nRemove: 2, 5, 7-9',
    exampleOutput: 'document-edited.pdf (5 pages remaining)',
    faqItems: [
      { question: 'Can I remove all pages?', answer: 'No. At least one page must remain in the output PDF.' },
      { question: 'How do I specify page ranges?', answer: 'Enter comma-separated numbers and ranges, e.g. "2, 5, 7-9" removes pages 2, 5, 7, 8, and 9.' },
      { question: 'Are page numbers 1-based?', answer: 'Yes. Page 1 is the first page of the PDF, matching how most PDF viewers label pages.' },
      { question: 'Is the original file modified?', answer: 'No. You download a new PDF. The original file on your device stays unchanged.' },
    ],
    relatedToolSlugs: ['pdf-merger', 'pdf-splitter', 'word-counter'],
    type: 'custom',
    useCases: ['Removing blank pages', 'Deleting confidential pages', 'Trimming unwanted sections'],
    recentlyAdded: true,
    useCaseTags: ['conversion', 'productivity'],
  },
  {
    ...
    category: 'Document Tools',
    categorySlug: 'document-tools',
    description: 'Split a PDF into smaller files by page count or custom ranges.',
    keywords: ['pdf splitter', 'split pdf', 'divide pdf', 'extract pages'],
    seoTitle: 'PDF Splitter Online — Split PDF Files Free',
    metaDescription: 'Split a PDF into smaller files every N pages or by custom page ranges. Free online PDF splitter — runs in your browser.',
    introText: 'Upload a PDF and split it into multiple files. Split every N pages for equal chunks, or define custom ranges like "1-3, 5-7" to extract specific sections.',
    exampleInput: 'Upload: manual.pdf (12 pages)\nSplit every: 4 pages',
    exampleOutput: 'manual-part-1.pdf (4 pages)\nmanual-part-2.pdf (4 pages)\nmanual-part-3.pdf (4 pages)',
    faqItems: [
      { question: 'How does "every N pages" work?', answer: 'The PDF is divided into consecutive chunks of N pages. The last chunk may have fewer pages if the total is not evenly divisible.' },
      { question: 'Can I extract specific page ranges?', answer: 'Yes. Switch to Page ranges mode and enter ranges like "1-3, 8-10" to download each range as a separate PDF.' },
      { question: 'Are downloads zipped?', answer: 'Files download individually. Your browser may ask to allow multiple downloads.' },
      { question: 'What if N is larger than the page count?', answer: 'The tool will show an error. N must be between 1 and the total number of pages.' },
    ],
    relatedToolSlugs: ['pdf-merger', 'pdf-page-remover', 'word-counter'],
    type: 'custom',
    useCases: ['Extracting chapters', 'Splitting large scans', 'Sharing specific sections'],
    featured: true,
    recentlyAdded: true,
    useCaseTags: ['conversion', 'productivity'],
  },
  {
    ...
    keywords: ['word count', 'character count', 'reading time', 'document counter'],
    seoTitle: 'Word Counter Online — Count Words & Reading Time Free',
    metaDescription: 'Count words, characters, sentences, paragraphs, and estimated reading time instantly. Free online word counter for documents and essays.',
    introText: 'Paste any document text and get word count, character count, sentence and paragraph counts, plus estimated reading time. Ideal for essays, articles, and content with word limits.',
    exampleInput: 'The quick brown fox jumps over the lazy dog. This is a sample sentence for counting.\n\nA second paragraph adds more words.',
    exampleOutput: 'Words: 20\nCharacters: 118\nCharacters (no spaces): 98\nSentences: 2\nParagraphs: 2\nReading time: ~1 min',
    faqItems: [
      { question: 'How are words counted?', answer: 'Words are split on whitespace. Hyphenated words count as one word.' },
      { question: 'How is reading time calculated?', answer: 'Reading time uses an average speed of 225 words per minute, rounded up to the nearest minute.' },
      { question: 'Does it count punctuation?', answer: 'Punctuation is included in character counts but not in word counts.' },
      { question: 'Can I use this for long documents?', answer: 'Yes. The tool handles large pasted text instantly in your browser.' },
    ],
    relatedToolSlugs: ['pdf-merger', 'character-counter', 'sentence-counter'],
    type: 'standard',
    useCases: ['Essay word limits', 'Article length tracking', 'Reading time estimates'],
    featured: true,
    popular: true,
    recentlyAdded: true,
    useCaseTags: ['counting', 'productivity'],
  },
  // ===== IMAGE TOOLS =====
  ...
  // ===== CALCULATORS =====
  {
    ...
    description: 'Convert Unix timestamps and date strings with relative time.',
    keywords: ['unix timestamp', 'epoch converter', 'timestamp to date', 'milliseconds'],
    seoTitle: 'Unix Timestamp Converter — Epoch to Date Online Free',
    metaDescription: 'Convert Unix timestamps (seconds or milliseconds) to dates and back. Shows ISO 8601, UTC, local time, and relative time. Free online converter.',
    introText: 'Paste a Unix timestamp or human-readable date to get ISO 8601, UTC, local time, relative time ("3 days ago"), and both seconds and milliseconds epoch values. Auto-detects seconds vs milliseconds.',
    exampleInput: '1700000000',
    exampleOutput: 'ISO 8601: 2023-11-14T22:13:20.000Z\nUTC: Tue, 14 Nov 2023 22:13:20 GMT\nLocal: 11/14/2023, 5:13:20 PM\nRelative: 2 years ago\nUnix (seconds): 1700000000\nUnix (milliseconds): 1700000000000',
    faqItems: [
      { question: 'Seconds vs milliseconds?', answer: '10-digit values are treated as seconds. 13-digit values are milliseconds. The tool auto-detects by digit length.' },
      { question: 'Can I paste a date string?', answer: 'Yes. Formats like "2024-06-15", ISO strings, and "Jun 15, 2024 2:30 PM" are supported.' },
      { question: 'What is relative time?', answer: 'Relative time shows how far the date is from now, e.g. "in 3 days" or "2 hours ago".' },
      { question: 'Is my data sent to a server?', answer: 'No. All conversion runs in your browser.' },
    ],
    relatedToolSlugs: ['cron-expression-calculator', 'data-size-converter', 'timestamp-converter', 'json-formatter'],
    type: 'standard',
    useCases: ['API timestamp debugging', 'Log file analysis', 'Database epoch conversion'],
    featured: true,
    popular: true,
    recentlyAdded: true,
    useCaseTags: ['conversion', 'debugging'],
  },
  {
    ...
    slug: 'cron-expression-calculator',
    category: 'Calculators',
    categorySlug: 'calculators',
    description: 'Parse cron expressions into human-readable schedules and next run times.',
    keywords: ['cron calculator', 'cron parser', 'cron expression', 'schedule'],
    seoTitle: 'Cron Expression Calculator — Parse Cron Online Free',
    metaDescription: 'Parse standard 5-field cron expressions. Get human-readable descriptions and the next 5 run times. Free online cron calculator.',
    introText: 'Enter a standard cron expression (minute hour day month weekday) to see a plain-English description and the next 5 scheduled run times from now.',
    exampleInput: '30 14 * * 1',
    exampleOutput: 'Expression: 30 14 * * 1\nDescription: At 2:30 PM, on Monday\n\nNext 5 run times (local):\n1. ...',
    faqItems: [
      { question: 'What cron format is supported?', answer: 'Standard 5-field Unix cron: minute (0-59), hour (0-23), day of month (1-31), month (1-12), weekday (0-6, 0=Sunday).' },
      { question: 'Does it support ranges and steps?', answer: 'Yes. Supports *, lists (1,3,5), ranges (1-5), and steps (*/15, 1-30/5).' },
      { question: 'What timezone are run times in?', answer: 'Next run times are shown in your browser\'s local timezone and ISO 8601 UTC.' },
      { question: 'Is this for systemd or Quartz cron?', answer: 'This tool uses standard 5-field cron. Quartz 6-field and systemd calendar formats are not supported.' },
    ],
    relatedToolSlugs: ['unix-timestamp-converter', 'data-size-converter', 'color-contrast-checker'],
    type: 'standard',
    useCases: ['CI/CD schedule validation', 'Cron job debugging', 'Backup schedule planning'],
    featured: true,
    recentlyAdded: true,
    useCaseTags: ['debugging', 'productivity'],
  },
  {
    ...
    seoTitle: 'Color Contrast Calculator — WCAG Ratio Checker Free',
    metaDescription: 'Calculate WCAG contrast ratio between two colors. Check AA and AAA pass/fail for normal and large text. Free online accessibility checker.',
    introText: 'Enter foreground and background hex colors to calculate the WCAG contrast ratio. See pass/fail for AA and AAA levels with a live text preview swatch.',
    exampleInput: '#FFFFFF\n#1A1A2E',
    exampleOutput: 'Contrast ratio: 15.42:1\nWCAG AA (normal text, 4.5:1): PASS\nWCAG AA (large text, 3:1): PASS\nWCAG AAA (normal text, 7:1): PASS\nWCAG AAA (large text, 4.5:1): PASS',
    faqItems: [
      { question: 'What is WCAG AA?', answer: 'WCAG AA requires 4.5:1 contrast for normal text and 3:1 for large text (18pt+ or 14pt bold).' },
      { question: 'What is WCAG AAA?', answer: 'WCAG AAA is stricter: 7:1 for normal text and 4.5:1 for large text.' },
      { question: 'How is contrast ratio calculated?', answer: 'Using relative luminance per WCAG 2.x: (Llighter + 0.05) / (Ldarker + 0.05).' },
      { question: 'Which color is foreground?', answer: 'Foreground is the text color. Background is the surface behind the text.' },
    ],
    relatedToolSlugs: ['color-converter', 'image-color-picker', 'cron-expression-calculator', 'unix-timestamp-converter'],
    type: 'custom',
    useCases: ['Accessible UI design', 'CSS color validation', 'Brand palette auditing'],
    popular: true,
    recentlyAdded: true,
    useCaseTags: ['debugging', 'productivity'],
  },
  {
    name: 'Storage / Data Size Converter',
    slug: 'data-size-converter',
    category: 'Calculators',
    categorySlug: 'calculators',
    description: 'Convert between binary (KiB/MiB/GiB) and decimal (KB/MB/GB) storage units.',
    keywords: ['data size converter', 'bytes converter', 'KiB to GB', 'storage calculator'],
    seoTitle: 'Data Size Converter — Bytes, KB, MB, GB Online Free',
    metaDescription: 'Convert storage units between binary (KiB, MiB, GiB) and decimal (KB, MB, GB) systems. Free online data size converter.',
    introText: 'Convert storage sizes between bytes, KB/MB/GB/TB (decimal, base 1000) or KiB/MiB/GiB/TiB (binary, base 1024). Enter value, from unit, to unit, and system on separate lines.',
    exampleInput: '1024\nMiB\nGiB\nbinary',
    exampleOutput: '1024 MiB (binary)\n= 1 GiB\nExact: 1\nIn bytes: 1073741824\nSystem: Binary (base 1024)',
    faqItems: [
      { question: 'Binary vs decimal?', answer: 'Binary (KiB/MiB/GiB) uses base 1024. Decimal (KB/MB/GB) uses base 1000. Storage vendors often use decimal; OS tools often use binary.' },
      { question: 'What units are supported?', answer: 'Binary: B, KiB, MiB, GiB, TiB. Decimal: B, KB, MB, GB, TB.' },
      { question: 'How do I specify the system?', answer: 'Add "binary" or "decimal" on the 4th line. Defaults to binary if omitted.' },
      { question: 'Why do 1000 GB and 1 TB differ?', answer: 'In decimal they are equal (1000³). In binary, 1 GiB = 1024 MiB, so conversions depend on which system you choose.' },
    ],
    relatedToolSlugs: ['unix-timestamp-converter', 'cron-expression-calculator', 'color-contrast-checker'],
    type: 'standard',
    useCases: ['Storage capacity planning', 'Bandwidth estimation', 'File size comparisons'],
    recentlyAdded: true,
    useCaseTags: ['conversion', 'productivity'],
  },
];
